...
export const registerWithPayment = async (req, res) => {
  ...
  try {
    ...
    const outsetaCheckoutUrl = 
      `https://juco-pipeline-2.outseta.com/auth` +
      `?widgetMode=register` +
      `&planUid=${plan}` +
      `&email=${encodeURIComponent(email)}` +
      `&firstName=${encodeURIComponent(firstName)}` +
      `&lastName=${encodeURIComponent(lastName)}` +
      `&Account.pendingRegistrationId=${pendingReg._id}` + // ← YEH KEY HAI
      `#o-anonymous`;
    ...
    res.status(201).json({
      ...
      outsetaCheckoutUrl: outsetaCheckoutUrl,
      plan
    });
