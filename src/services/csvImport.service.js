...
/**
 * Read and parse CSV or XLSX file
 */
 ...
 */
const findOrCreateTeam = async (collegeName) => {
  if (!collegeName) {
  ...
  if (!team) {
    ...
    console.log(`Created new team: ${collegeName}`);
  }
...
/**
 * Import players from CSV or XLSX file
 */
export const importPlayersFromCSV = async (filePath) => {
  try {
    // Read file (supports both CSV and XLSX)
    ...
    // Track created teams to avoid duplicate console logs
    ...
    for (const record of records) {
      try {
        ...
        // Find or create team
        const team = await findOrCreateTeam(collegeName);
        
        // Track if this is a newly created team
        if (!createdTeams.has(team._id.toString())) {
          const isNewTeam = team.createdAt && 
            (Date.now() - team.createdAt.getTime() < 5000); // Created in last 5 seconds
          
          if (isNewTeam) {
            results.teamsCreated++;
            createdTeams.add(team._id.toString());
          }
        }
        ...
        if (playerData.highSchool?.trim()) {
          playerData.commitmentStatus = 'committed';
        }
